...
"@forthtilliath/expo-release-updates-ui": minor
---

**Breaking:** `ChangelogNotes`, `UpdateAvailableBanner` and `useUpdateCheck` moved out of `@forthtilliath/react-native-kit` into the new `@forthtilliath/expo-release-updates-ui` package — install it alongside `react-native-kit` if you use them. `UpdateSettingsScreen` now renders release notes via that package internally.
